...
func TestContextWindow(t *testing.T) {
	cases := []struct {
		...
		{"gpt-5.1", 400_000},            // exact
		{"gpt-5.1-2025-11-13", 400_000}, // dated snapshot → gpt-5.1
		{"gpt-5.10", 0},                 // not a gpt-5.1 family member
		{"gpt-4o", 128_000},
...
func TestDefaultModelHasKnownContextWindow(t *testing.T) {
	if DefaultModel != "gpt-5.1" {
		t.Fatalf("DefaultModel = %q, want gpt-5.1", DefaultModel)
	}
	if got := ContextWindow(DefaultModel); got != 400_000 {
		t.Fatalf("ContextWindow(DefaultModel %q) = %d, want 400000", DefaultModel, got)
	}
}

...
func TestAuthoring_EndToEnd(t *testing.T) {
	...
	exec := NewExecutor(prov, t.TempDir(), "", "", nil, reg, nil, nil, rec, capability.TierBalanced, nil)
...
func TestAuthoring_TestFailureSurfacedToModel(t *testing.T) {
	...
	reg := tools.NewMemoryRegistry()
	exec := NewExecutor(prov, t.TempDir(), "", "", nil, reg, nil, nil, &audit.MemoryRecorder{}, capability.TierBalanced, nil)

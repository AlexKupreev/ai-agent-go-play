...
func TestSelfDocs_EndToEnd(t *testing.T) {
	...
	obs := &captureObserver{}
	exec := NewExecutor(prov, t.TempDir(), "", "", obs, tools.NewMemoryRegistry(), nil, docs, &audit.MemoryRecorder{}, capability.TierBalanced, nil)
...
func TestSelfDocs_OmittedWhenNil(t *testing.T) {
	...
	exec := NewExecutor(prov, t.TempDir(), "", "", nil, tools.NewMemoryRegistry(), nil, nil, &audit.MemoryRecorder{}, capability.TierBalanced, nil)

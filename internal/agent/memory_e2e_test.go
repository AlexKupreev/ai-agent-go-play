...
func TestMemory_PersistsAcrossRuns(t *testing.T) {
	...
	exec1 := NewExecutor(run1, t.TempDir(), "", "run-1", nil, tools.NewMemoryRegistry(), store, nil, rec, capability.TierBalanced, nil)
	...
	exec2 := NewExecutor(run2, t.TempDir(), "", "run-2", obs, tools.NewMemoryRegistry(), store, nil, rec, capability.TierBalanced, nil)

...
- **Interactive chat** — `agent chat` is a multi-turn REPL that retains conversation context
  across turns (`/new` or `/reset`, `/exit`). Each message is **deliberate by default**: a
  **planner** refines it, an **executor** runs it, and a **critic** judges the result and re-plans
  a shortfall (`--no-plan` / `--no-critique` drop back to a bare executor). Add `--addr` to drive
...
- **Headless engine** — `agent serve` runs the engine as a local HTTP+SSE service; `agent client
  <task>` (and, optionally, a Telegram bot) drive it as peer clients, with risky actions parking
  for a remote approval. Session turns are deliberate by default (same `--no-plan` / `--no-critique`
  dials); one-shot `POST /runs` runs the executor directly. Good for unattended use.

...
- [x] **Implicit recall / switchable contexts → Spaces — BUILT (P1+P2+P3-guidance, 2026-07-11).**
    Switchable *data* scopes (per-space memory + always-loaded guidance = the per-space
    ...
    trusted tools (`list_spaces`/`create_space`/`switch_space`/`space_guidance`/`update_space_guidance`),
    guidance injected as a labelled prompt append. **Includes the governing-decision move of memory to
